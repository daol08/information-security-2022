...
def sdes_encrypt_ecb(text: bitarray, key: bitarray):
    cypher_text = bitarray()
    block_list = [text[i:i+8] for i in range(0, len(text), 8)]
    for block in block_list:
        block_cypher = sdes(block, key, MODE_ENCRYPT)
        cypher_text.extend(block_cypher)
    return cypher_text

def sdes_decrypt_ecb(ciphertext: bitarray, key: bitarray):
    de_text = bitarray()
    block_list = [ciphertext[i:i+8] for i in range(0, len(ciphertext), 8)]
    for block in block_list:
        block_de = sdes(block, key, MODE_DECRYPT)
        de_text.extend(block_de)
    return de_text
...
def sdes_encrypt_cbc(text: bitarray, key: bitarray, iv:bitarray):
    ...
    for block in block_list:
        ...
        if block == block_list[0]:
            block_cypher = block ^ iv
            block_cypher = sdes(block_cypher, key, MODE_ENCRYPT)
            cypher_text.extend(block_cypher)
            before = block_cypher
        else:
            block_cypher = block ^ before
            block_cypher = sdes(block_cypher, key, MODE_ENCRYPT)
            cypher_text.extend(block_cypher)
            before = block_cypher
    return cypher_text
...
def sdes_decrypt_cbc(ciphertext: bitarray, key: bitarray, iv:bitarray):
    ...
    for block in block_list:
        ...
        if block == block_list[0]:
            block_de = sdes(block, key, MODE_DECRYPT)
            block_de= block_de ^ iv
            de_text.extend(block_de)
            before = block_de
        else:
            block_de = sdes(block, key, MODE_DECRYPT)
            block_de = block_de ^ before
            de_text.extend(block_de)
            before = block_de
    return de_text
...
#처음 코드에서 9bit으로 생성되어서 코드를 약간 바꿨습니다.
